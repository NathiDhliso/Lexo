-- Fix practice health metrics function to work without parameters
-- and return the structure expected by the frontend
...
BEGIN
  ...
  SELECT 
    ...
    -- Simple health trend calculation (could be enhanced with historical data)
    CASE 
    ...
    CASE 
      WHEN s.total_matters = 0 THEN 0
      ELSE GREATEST(0, LEAST(100, 
        100 - (s.matters_90_plus::DECIMAL / s.total_matters * 50) - 
    ...
    CASE 
      ...
      ELSE LEAST(100, 
        ...
        (s.matters_90_plus::DECIMAL / s.total_matters * 30)
      )
...
COMMENT ON FUNCTION get_practice_health_metrics IS 'Provides comprehensive practice health metrics for dashboard display without requiring parameters';