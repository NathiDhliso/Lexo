...
const getTypeStyles = (type: SmartNotification['type']) => {
  switch (type) {
    case 'urgent':
      return 'border-status-error-200 dark:border-status-error-700 bg-status-error-50 dark:bg-status-error-900/20';
    case 'attention':
      return 'border-status-warning-200 dark:border-status-warning-700 bg-status-warning-50 dark:bg-status-warning-900/20';
    case 'success':
      return 'border-status-success-200 dark:border-status-success-700 bg-status-success-50 dark:bg-status-success-900/20';
    default:
      return 'border-neutral-200 dark:border-metallic-gray-700 bg-neutral-50 dark:bg-metallic-gray-800';
  }
};
    ...
    <div className="absolute right-0 mt-2 w-96 bg-white dark:bg-metallic-gray-900 border border-neutral-200 dark:border-metallic-gray-700 rounded-lg theme-shadow-lg z-50">
      <div className="flex items-center justify-between px-3 py-2 border-b border-neutral-200 dark:border-metallic-gray-700">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-neutral-900 dark:text-neutral-100">Alerts</span>
        </div>
        <button className="p-1 hover:bg-neutral-100 dark:hover:bg-metallic-gray-800 rounded" onClick={onClose} aria-label="Close alerts">
      ...
      <div className="max-h-80 overflow-y-auto">
        {notifications.length === 0 ? (
          <div className="p-4 text-center text-sm text-neutral-600 dark:text-neutral-400">No alerts right now</div>
        ) : (
            ...
            <div key={n.id} className={`px-3 py-2 border-t ${getTypeStyles(n.type)} flex items-start gap-3`}> 
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-neutral-500 dark:text-neutral-400">{new Date(n.timestamp).toLocaleString()}</span>
                </div>
                <p className="text-sm text-neutral-700 dark:text-neutral-300 mt-1">{n.message}</p>

                <div className="mt-2 flex items-center gap-2">
                  {!n.read && (
                    <button className="text-xs px-2 py-1 bg-neutral-100 dark:bg-metallic-gray-800 hover:bg-neutral-200 dark:hover:bg-metallic-gray-700 dark:text-neutral-200 rounded" onClick={() => markRead(n.id)}>Mark read</button>
                  )}
                  <button className="text-xs px-2 py-1 bg-neutral-100 dark:bg-metallic-gray-800 hover:bg-neutral-200 dark:hover:bg-metallic-gray-700 dark:text-neutral-200 rounded" onClick={() => dismiss(n.id)}>Dismiss</button>
                  {n.actionable && (
                    <button className="text-xs px-2 py-1 bg-mpondo-gold-100 dark:bg-mpondo-gold-900/30 text-mpondo-gold-700 dark:text-mpondo-gold-300 hover:bg-mpondo-gold-200 dark:hover:bg-mpondo-gold-900/50 rounded inline-flex items-center gap-1" onClick={() => handleAction(n)}>
