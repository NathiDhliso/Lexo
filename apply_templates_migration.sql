-- Run this script in Supabase SQL Editor to enable templates functionality
-- This checks if tables exist before creating them

-- Create template categories table if not exists
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'template_categories') THEN
...
-- Create matter templates table if not exists
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'matter_templates') THEN
...
-- Create template sharing table if not exists
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'template_shares') THEN
...
-- Add template_id to matters table if not exists
DO $$ 
BEGIN
  IF NOT EXISTS (
...
DROP TRIGGER IF EXISTS update_matter_templates_updated_at ON matter_templates;
...
DROP POLICY IF EXISTS "Users can manage own templates" ON matter_templates;
DROP POLICY IF EXISTS "Users can view shared templates" ON matter_templates;
DROP POLICY IF EXISTS "Users can manage own template shares" ON template_shares;
DROP POLICY IF EXISTS "Users can view received shares" ON template_shares;
DROP POLICY IF EXISTS "Everyone can read template categories" ON template_categories;
DROP POLICY IF EXISTS "Authenticated users can manage categories" ON template_categories;
...
-- Create or replace function to get user's templates with sharing info
...
-- Create or replace function to suggest templates based on matter data
...
UNION ALL
SELECT 
  'matter_templates' as table_name,
  COUNT(*) as row_count
FROM matter_templates
UNION ALL
SELECT 
  'template_shares' as table_name,
  COUNT(*) as row_count
